  ...
  return (
    <div className="py-2 flex flex-col w-full bg-transparent mb-2 duration-100 border-b border-primary hover:border-secondary">
      <Link href={`/stories/${id}`} passHref>
        <h3
          className={`text-lg text-secondary whitespace-pre-line font-medium duration-100 cursor-pointer font-serif hover:text-primary`}
        >
          {decode(title)}{" "}
        </h3>
      </Link>
      {domain && (
        <a
          ...
          className="text-base whitespace-nowrap font-normal mb-0.5 border-b hover:text-primary border-primary w-fit font-serif text-secondary mt-0.5"
        >
          ({domain})
        </a>
      )}
